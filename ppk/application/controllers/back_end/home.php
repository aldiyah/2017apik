...
class Home extends Back_end {
    ...
    public function index() {
        $tahun = date('Y');
        $bulan = date('n');
        $this->load->model(array('model_tr_skp_bulanan', 'model_tr_perilaku', 'model_master_tpp'));
        $data_tpp = $this->model_master_tpp->get_detail('master_tpp.pegawai_id = ' . $this->pegawai_id);
        $tpp_ppk = $data_tpp ? $data_tpp->tpp_beban_kerja * $this->config->item('perwal')['ppk']['bobot'] : 0;
        $skpb = $this->model_tr_skp_bulanan->get_all_skpb_by_id($this->pegawai_id, $tahun, $bulan)->record_set;
        $nilai_capaian = 0;
        $nilai_perilaku = 0;
        $jml_terima = 0;
        $jml_tolak = 0;
        ...
        if ($skpb) {
            foreach ($skpb as $row) {
                $skpb_status = $row->skpb_status;
                switch ($skpb_status) {
                    case 2:
                        $nilai_capaian += $row->skpb_nilai;
                        $jml_terima++;
                        $jumlah++;
                        break;
                    case 3:
        ...
        $perilaku = $this->model_tr_perilaku->get_perilaku_by_id($this->pegawai_id, $tahun, $bulan);
        if ($perilaku) {
            if ($perilaku->perilaku_kepemimpinan > 0) {
                $nilai_perilaku = ($perilaku->perilaku_pelayanan + $perilaku->perilaku_integritas + $perilaku->perilaku_komitmen + $perilaku->perilaku_disiplin + $perilaku->perilaku_kerjasama + $perilaku->perilaku_kepemimpinan) / 6;
            } else {
        ...
        $nilai_final = ((0.6 * ($jumlah > 0 ? $nilai_capaian / $jumlah : 0)) + (0.4 * $nilai_perilaku));
        $nilai_tpp = 0;
        if ($nilai_final <= 0) {
            $nilai_tpp = 0.0 * $tpp_ppk;
        } elseif ($nilai_final < 15) {
            $nilai_tpp = 0.2 * $tpp_ppk;
        } elseif ($nilai_final < 25) {
            $nilai_tpp = 0.3 * $tpp_ppk;
        } elseif ($nilai_final < 35) {
            $nilai_tpp = 0.4 * $tpp_ppk;
        } elseif ($nilai_final < 45) {
            $nilai_tpp = 0.5 * $tpp_ppk;
        } elseif ($nilai_final < 55) {
            $nilai_tpp = 0.6 * $tpp_ppk;
        } elseif ($nilai_final < 65) {
            $nilai_tpp = 0.7 * $tpp_ppk;
        } elseif ($nilai_final < 75) {
            $nilai_tpp = 0.8 * $tpp_ppk;
        } elseif ($nilai_final < 85) {
            $nilai_tpp = 0.9 * $tpp_ppk;
        } else {
            $nilai_tpp = 1.0 * $tpp_ppk;
        }

//        var_dump($this->perwal, $tpp_ppk, $jml_terima, $jml_tolak, $nilai_capaian, $nilai_perilaku, $nilai_final, $nilai_tpp);
//        exit();
        $this->set("total_tpp", $nilai_tpp);
        $this->set("total_skp_diterima", $jml_terima);
        $this->set("total_skp_ditolak", $jml_tolak);
    }
