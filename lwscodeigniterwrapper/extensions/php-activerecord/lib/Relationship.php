...
interface InterfaceRelationship
{
	...
	public function build_association(AR_model $model, $attributes=array());
	public function create_association(AR_model $model, $attributes=array());
}
...
abstract class AbstractRelationship implements InterfaceRelationship
{
	 ...
	 *
	 * @param AR_model $model The model which holds this association
	...
	protected function append_record_to_associate(AR_model $associate, AR_model $record)
	{
	...
	protected function unset_non_finder_options($options)
	{
		...
		{
			if (!in_array($option, AR_model::$VALID_OPTIONS))
	...
	protected function set_class_name($class_name)
	{
		...
		if (!$reflection->isSubClassOf('AR_model'))
        {
			throw new RelationshipException("'$class_name' must extend from AR_model");
        }

		$this->class_name = $class_name;
	}

	protected function create_conditions_from_keys(AR_model $model, $condition_keys=array(), $value_keys=array())
	{
	 ...
	 *
	 * @param AR_model $model The model this relationship belongs to
	 */
	abstract function load(AR_model $model);
};
...
class HasMany extends AbstractRelationship
{
	...
	public function load(AR_model $model)
	{
		$class_name = $this->class_name;
	...
	private function inject_foreign_key_for_new_association(AR_model $model, &$attributes)
	{
		$this->set_keys($model);
