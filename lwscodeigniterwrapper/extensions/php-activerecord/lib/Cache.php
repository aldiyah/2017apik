...
class Cache {
    ...
    public static function initialize($url, $options=array())
    {
        ...
        self::$adapter = $CI->cache != NULL ? $CI->cache : NULL;
//		if ($url)
//		{
//			$url = parse_url($url);
...
//			static::$adapter = null;
		...
		static::$options = array_merge(array('expire' => 604800, 'namespace' => ''),$options);
	}
    ...
    public static function get($key, $closure)
    {
        ...
        if (!($value = static::$adapter->get($key)))
            static::$adapter->save($key, ($value = $closure()), static::$options['expire']);
